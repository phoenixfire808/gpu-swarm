...
Copy everything inside the fence below into Discord. Replace the public portal URL with the live one from Drew (`data/public_endpoints.share.txt` on the host).
...
1. Open the public link Drew posts (trycloudflare.com), e.g.:
   https://REPLACE-WITH-LIVE.trycloudflare.com/portal
2. Sign in: invite **glitch-factor** + your Discord display name
3. **Utilize** allowlisted jobs — or **Contribute** (GPU or CPU-only / VRAM=0)
4. Optional SDK/CLI: set GPU_SWARM_SCHEDULER_URL to the same host + /pool-api
...
| Point | Detail |
|-------|--------|
| Public first | Open Drew’s `…trycloudflare.com/portal` — **no Tailscale needed** while the tunnel is up |
...
DM friends the portal line from `data\public_endpoints.share.txt`.
